...
const showedQuestions= []
 ...
 const answerConteiner = document.querySelector (".answers");
...
const scoreResult = resultScreen.querySelector(".score");
 ...
 const selectAnswers = () => {
  answerConteiner.querySelectorAll("input").forEach(element =>{
 ...
 const showQuestion = (index) => {
  ...
  if(index === data.lenght){
    ...
    answerConteiner.innerHTML  = data[index].answers.map((item, index )=>
     `<div class= "answer" > 
...
const showResult = () => {
  ...
  scoreResult.textContent = `Result : ${(correctCount - wrongCount) * 10 }`
};
